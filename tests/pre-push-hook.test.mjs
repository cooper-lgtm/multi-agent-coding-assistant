...
`,
    ...
    assert.deepEqual(capturedArgs, ['--head-range', 'develop', featureSha]);
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});

test('pre-push hook reviews the pushed branch sha instead of the checked-out HEAD', async () => {
  const tempRoot = await mkdtemp(path.join(tmpdir(), 'pre-push-hook-pushed-sha-'));
  const hookTargetPath = path.join(tempRoot, '.githooks', 'pre-push');
  const fakeReviewScriptPath = path.join(tempRoot, 'scripts', 'run-local-codex-review.mjs');
  const capturePath = path.join(tempRoot, 'review-command.json');
  const sourceHookPath = path.join(process.cwd(), '.githooks', 'pre-push');
...
`,
    ...
    await writeFile(path.join(tempRoot, 'README.md'), 'base\nfeature\n', 'utf8');
    execFileSync('git', ['add', 'README.md'], { cwd: tempRoot, stdio: 'pipe' });
    execFileSync('git', ['commit', '-m', 'feature'], { cwd: tempRoot, stdio: 'pipe' });
    const featureSha = execFileSync('git', ['rev-parse', 'HEAD'], {
      cwd: tempRoot,
      encoding: 'utf8',
      stdio: 'pipe',
    }).trim();
    execFileSync('git', ['checkout', 'main'], { cwd: tempRoot, stdio: 'pipe' });
    ...
    const capturedArgs = JSON.parse(await readFile(capturePath, 'utf8'));
    assert.deepEqual(capturedArgs, ['--head-range', 'main', featureSha]);
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});

test('pre-push hook falls back to the remote default branch when main and master are absent', async () => {
  const tempRoot = await mkdtemp(path.join(tmpdir(), 'pre-push-hook-remote-default-'));
  const hookTargetPath = path.join(tempRoot, '.githooks', 'pre-push');
  const fakeReviewScriptPath = path.join(tempRoot, 'scripts', 'run-local-codex-review.mjs');
  const capturePath = path.join(tempRoot, 'review-command.json');
  const sourceHookPath = path.join(process.cwd(), '.githooks', 'pre-push');
...
`,
    ...
    execFileSync('git', ['init', '--initial-branch=trunk'], { cwd: tempRoot, stdio: 'pipe' });
    execFileSync('git', ['config', 'user.name', 'Codex Test'], { cwd: tempRoot, stdio: 'pipe' });
    execFileSync('git', ['config', 'user.email', 'codex@example.com'], { cwd: tempRoot, stdio: 'pipe' });
    await writeFile(path.join(tempRoot, 'README.md'), 'base\n', 'utf8');
    execFileSync('git', ['add', 'README.md'], { cwd: tempRoot, stdio: 'pipe' });
    execFileSync('git', ['commit', '-m', 'init'], { cwd: tempRoot, stdio: 'pipe' });
    const trunkSha = execFileSync('git', ['rev-parse', 'HEAD'], {
      cwd: tempRoot,
      encoding: 'utf8',
      stdio: 'pipe',
    }).trim();
    execFileSync('git', ['update-ref', 'refs/remotes/origin/trunk', trunkSha], { cwd: tempRoot, stdio: 'pipe' });
    execFileSync('git', ['symbolic-ref', 'refs/remotes/origin/HEAD', 'refs/remotes/origin/trunk'], { cwd: tempRoot, stdio: 'pipe' });
    execFileSync('git', ['checkout', '-b', 'feature'], { cwd: tempRoot, stdio: 'pipe' });
    ...
    const capturedArgs = JSON.parse(await readFile(capturePath, 'utf8'));
    assert.deepEqual(capturedArgs, ['--head-range', 'origin/trunk', featureSha]);
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});
