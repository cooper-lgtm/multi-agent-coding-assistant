...
5. If local required verification passed, merge without waiting for the automatic GitHub-hosted Codex review workflow to finish:
   - `gh pr merge --merge --delete-branch`
6. Refresh any repository context artifacts that the active Goose recipe requires after merge.
7. The GitHub-hosted Codex review workflow may still run asynchronously for comparison, follow-up fixes, and signaling.
...
1. Run the local blocking Codex review gate after required checks pass.
2. If local review returns findings, rerun the same task with the normalized review feedback.
3. If local review fails because of timeout/auth/process/schema issues, stop as `manual_review_required`.
4. Merge only after both required local checks and the local blocking review pass cleanly.
5. Keep GitHub-hosted Codex review as an optional asynchronous comparison signal rather than the Goose repair-loop source of truth.
